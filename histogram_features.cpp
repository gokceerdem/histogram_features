...
void features(ImData &mid, int seg_no){
	...
	//for (int k = 0; k< mid.connComp.size(); k++){
		...
		for (int s = 0; s < mid.connComp.at(k).size(); s++){
			...
			double	dumVal54 = mid.filter.at(30).at<uchar>(mid.connComp.at(k).at(s).y, mid.connComp.at(k).at(s).x);
...
float labels_array[15] = { 0 };
float trainingData[15][numberofbins] = { 0 };
float samples[10][numberofbins] = { 0 };
...
int index = 0;
int sindex = 0;
int train_sample_number, test_sample_number;

Mat filt_crop;
int histSize = 8;    // bin size
float range[] = { 0, 255 };
...
{	//tstart = time(0);
	...
	do{
		int image_no, segment_no, label_no;
		...
		cout << "Label : (1:monitor // -1:officechair)" << endl;
		cin >> label_no;

		hesapla(image_no, segment_no, label_no);
		index = index + 1;
	...
	do{
		int test_image_no, test_segment_no;
		cout << "Test Image no (10-106): ";
		cin >> test_image_no;

		cout << "Test Segment no : (max 150)";
		cin >> test_segment_no;

		testyap(test_image_no, test_segment_no);
		sindex = sindex + 1;
...
void hesapla(int image_no, int segment_no, int label_no){
		...
		//	saveFeatures(featureVector,myImData,ivar);
			...
			for (int filter_no = 0; filter_no < 30; filter_no++)
